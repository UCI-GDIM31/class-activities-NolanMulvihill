...
public class BatManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < _bats.Length; ++i)
        {
            _bats[i]._playerLocation = _playerTransform.position;
            if (Mathf.Abs(Vector3.Distance(_bats[i].GetComponent<Transform>().position, _playerTransform.position)) < _interactDistance)
            {
                if (Mathf.Abs(Vector3.Distance(_bats[i].GetComponent<Transform>().position, _playerTransform.position)) < _overlapDistance)
                {
    ...
    private void CreateReactions(BatW6 bat)
    {
        ...
        SpawnReactionUI(bat, _messages[Random.Range(0,_messages.Length - 1)]);
